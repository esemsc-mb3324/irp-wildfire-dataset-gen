...
def set_parameters():
    ...
    # get available fuel models from available_fuel_models.txt which has available fuel models going down the first column (one per line)
    with open('available_fuel_models.txt', 'r') as f:
        available_fuel_models = [int(line.strip()) for line in f if line.strip().isdigit()]
        print(available_fuel_models)
    fuel_model = np.random.choice(available_fuel_models)  # Randomly select from available fuel models
