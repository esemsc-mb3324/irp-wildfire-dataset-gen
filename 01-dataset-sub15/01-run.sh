...
NUM_FLOAT_RASTERS=7
FLOAT_RASTER[1]=ws   ; FLOAT_VAL[1]=15.5 # Wind speed, mph
FLOAT_RASTER[2]=wd   ; FLOAT_VAL[2]=9.1  # Wind direction, deg
FLOAT_RASTER[3]=m1   ; FLOAT_VAL[3]=4.8  # 1-hr   dead moisture content, %
FLOAT_RASTER[4]=m10  ; FLOAT_VAL[4]=7.6  # 10-hr  dead moisture content, %
FLOAT_RASTER[5]=m100 ; FLOAT_VAL[5]=10.5  # 100-hr dead moisture content, %
...
NUM_INT_RASTERS=8
INT_RASTER[1]=slp     ; INT_VAL[1]=38   # Topographical slope (deg) # perturbed directly
INT_RASTER[2]=asp     ; INT_VAL[2]=61   # Topographical aspect (deg) # perturbed directly
INT_RASTER[3]=dem     ; INT_VAL[3]=0   # Elevation (m)
INT_RASTER[4]=fbfm40  ; INT_VAL[4]=14 # Fire behavior fuel model code (-)
INT_RASTER[5]=cc      ; INT_VAL[5]=81   # Canopy cover (percent)
INT_RASTER[6]=ch      ; INT_VAL[6]=4   # Canopy height (10*meters)
INT_RASTER[7]=cbh     ; INT_VAL[7]=2   # Canopy base height (10*meters)
INT_RASTER[8]=cbd     ; INT_VAL[8]=22   # Canopy bulk density (100*kg/m3)

LH_MOISTURE_CONTENT=79.4 # Live herbaceous moisture content, percent
LW_MOISTURE_CONTENT=80.4 # Live woody moisture content, percent
